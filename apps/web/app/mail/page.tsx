...
import { MailNavbar } from "~/components/mail-navbar";
...
export default function MailInboxPage() {
  ...
  // Reset API pagination when search query or label filter changes.
  useEffect(() => {
    setPageToken(undefined);
    setPageTokenStack([]);
  }, [debouncedQuery, selectedLabelId]);
  ...
  function handleNextPage() {
    if (shouldUseApi) {
      if (apiData?.nextPageToken) {
        setPageTokenStack((prev) => (pageToken ? [...prev, pageToken] : prev));
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-t p-3">
        <Button
          variant="outline"
          size="sm"
          disabled={shouldUseApi ? pageTokenStack.length === 0 : dbOffset === 0}
